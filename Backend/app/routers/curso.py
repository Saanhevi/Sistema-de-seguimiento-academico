...
from app.core.dependencies import get_curso_service, get_session
...
def crear_grado(payload: GradoCreate, service: CursoService = Depends(get_curso_service)):
    return service.crear_grado(payload.nombre)


@router.get("/grados", response_model=list[GradoResponse])
def listar_grados(service: CursoService = Depends(get_curso_service)):
    return service.listar_grados()


@router.post("/materias", response_model=MateriaResponse)
def crear_materia(payload: MateriaCreate, service: CursoService = Depends(get_curso_service)):
    return service.crear_materia(payload.nombre)


@router.get("/materias", response_model=list[MateriaResponse])
def listar_materias(service: CursoService = Depends(get_curso_service)):
    return service.listar_materias()


@router.post("/periodos", response_model=PeriodoAcademicoResponse)
def crear_periodo(payload: PeriodoAcademicoCreate, service: CursoService = Depends(get_curso_service)):
...
def listar_periodos(service: CursoService = Depends(get_curso_service)):
    return service.listar_periodos()


@router.post("/cursos", response_model=CursoResponse)
def crear_curso(payload: CursoCreate, service: CursoService = Depends(get_curso_service)):
    return service.crear_curso(payload.id_docente, payload.id_grado, payload.id_materia, payload.id_periodo)
...
def obtener_curso(id_curso: int, service: CursoService = Depends(get_curso_service)):
    return service.obtener_curso(id_curso)


@router.post("/matriculas", response_model=MatriculaResponse)
def crear_matricula(payload: MatriculaCreate, service: CursoService = Depends(get_curso_service)):
    return service.crear_matricula(payload.id_estudiante, payload.id_grado, payload.anio)
